...
, {
  ...
  // =========================================================================
// 🚀 ORQUESTATOR DE INTERFAZ INTEGRAL - SOTO SYSTEM (CANDADO DE HERENCIA ÚNICA)
...
ErpModulo.init = function() {
    ...
    // 1. Renderizado nativo del panel del ERP en el lienzo secundario si está activo
    if (typeof contenedorInterno !== 'undefined' && contenedorInterno !== null) {
        ...
        // 🎯 2. 🖨️ EL CANDADO DE HERENCIA EXCLUSIVO:
        // Importamos y renderizamos el footer original usando tu función legítima del Home
        if (typeof Home !== 'undefined' && typeof Home.renderFooter === 'function') {
            
            // Evaluamos físicamente el lienzo interno para ver si el footer ya fue inyectado
            const yaExisteFooterInterno = contenedorInterno.querySelector('.soto-system-footer') || contenedorInterno.querySelector('footer');
            
            if (!yaExisteFooterInterno) {
                // 🚀 DISPARO DE HERENCIA LEGÍTIMO: Invocamos tu función original de toda la vida
                const footerOriginal = Home.renderFooter();
                contenedorInterno.appendChild(footerOriginal);
                console.log("📥 [SOTO CORE]: Footer original heredado e inyectado en contenedorInterno.");
            } else {
                console.log("🛡️ [SOTO CORE]: El footer ya existe en este contenedor. Bloqueada duplicación por herencia.");
            }
        }
        
    } else {
        // 🎯 3. BYPASS SEGURO PARA SUBSECCIONES HUÉRFANAS:
        ...
        if (typeof Home !== 'undefined' && typeof Home.renderFooter === 'function') {
            ...
            // Evaluamos la raíz general antes de disparar la herencia externa
            const yaExisteFooterGeneral = appContainerGeneral.querySelector('.soto-system-footer') || appContainerGeneral.querySelector('footer');
            
            if (!yaExisteFooterGeneral) {
                const footerOriginal = Home.renderFooter();
                appContainerGeneral.appendChild(footerOriginal);
                console.log("⚡ [SOTO CORE]: Activado bypass seguro. Footer heredado de forma única en la raíz general.");
            } else {
                console.log("🛡️ [SOTO CORE]: El footer ya existe en la raíz general. Cancelando clon.");
            }
        }
    }
};
